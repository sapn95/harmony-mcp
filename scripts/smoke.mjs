...
// handshake, lists the tools and asserts the things that actually break here: a
// version that drifted from package.json, a tool added to the dispatcher but not
// to the tool list (or the reverse), and a malformed inputSchema that only shows
// up when a client tries to call it.
//
// No network, no hub address — safe to run in CI.
...
// neither reads the developer's login keychain nor has an address to reach. Even
// if a check here grew into a real tool call, there is no hub for it to find.
const child = spawn(process.execPath, [entry], {
  stdio: ['pipe', 'pipe', 'pipe'],
  env: { ...process.env, HARMONY_HUB_HOST: '', HARMONY_HUB_REMOTE_ID: '', HARMONY_HUB_PORT: '' },
});
let stderr = '';
...
try {
  ...
  // Every advertised tool must actually be dispatched. Grepping the source for
  // the names that same source advertises is a tautology — green even while the
  // README lists tools that were deleted. Ask the server instead: "unknown tool"
  // is the one answer that proves no branch exists, and every other outcome
  // proves one does.
  //
  // That only holds because the dispatcher decides which branch handles a call
  // BEFORE it needs a hub. If the address lookup came first, every tool here
  // would answer "HARMONY_HUB_HOST ist nicht gesetzt" and a branch deleted from
  // the chain would leave this check green.
